...
class PayrollPDF(FPDF):
    def header(self):
        ...
        self.set_text_color(78, 54, 41)
        self.cell(0, 10, 'SLIP GAJI KARYAWAN', 0, 1, 'L')
        self.set_font('Arial', 'B', 10)
        self.set_text_color(43, 43, 43)
        self.cell(0, 5, 'KOPI SIBEI - MANAGEMENT SYSTEM', 0, 1, 'L')
        self.ln(10)

    def footer(self):
        self.set_y(-15)
        self.set_font('Arial', 'I', 8)
        self.set_text_color(142, 124, 119)
        self.cell(0, 10, 'Halaman ' + str(self.page_no()) + ' | Slip Gaji Resmi Kopi Sibei', 0, 0, 'C')

def generate_payroll_pdf(cashier_name, email, period, issued_date, rate, total_shifts, total_salary):
    pdf = PayrollPDF()
    pdf.add_page()
    ...
    pdf.cell(35, 7, 'Nama Karyawan:', 0, 0)
    pdf.set_font('Arial', '', 10)
    pdf.cell(60, 7, cashier_name, 0, 0)
    
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(30, 7, 'Periode Gaji:', 0, 0)
    pdf.set_font('Arial', '', 10)
    pdf.cell(60, 7, period, 0, 1)
    ...
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(30, 7, 'Tanggal Terbit:', 0, 0)
    pdf.set_font('Arial', '', 10)
    pdf.cell(60, 7, issued_date, 0, 1)
    
    pdf.ln(10)
    pdf.set_draw_color(225, 220, 214)
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(10)
    
    pdf.set_font('Arial', 'B', 11)
    pdf.set_text_color(78, 54, 41)
    pdf.cell(0, 8, 'Rincian Gaji', 0, 1, 'L')
    pdf.ln(2)
    
    pdf.set_font('Arial', 'B', 10)
    pdf.set_text_color(43, 43, 43)
    pdf.set_fill_color(252, 249, 246)
    pdf.cell(120, 8, ' Deskripsi', 1, 0, 'L', True)
    pdf.cell(70, 8, ' Jumlah', 1, 1, 'R', True)
    
    pdf.set_font('Arial', '', 10)
    pdf.cell(120, 8, ' Bayaran per Shift', 1, 0, 'L')
    pdf.cell(70, 8, f' Rp {rate:,} ', 1, 1, 'R')
    
    pdf.cell(120, 8, ' Total Shift Hadir', 1, 0, 'L')
    pdf.cell(70, 8, f' {total_shifts} Shift ', 1, 1, 'R')
    ...
    pdf.set_text_color(78, 54, 41)
    pdf.set_fill_color(252, 249, 246)
    pdf.cell(120, 8, ' TOTAL GAJI DITRANSFER', 1, 0, 'L', True)
    pdf.cell(70, 8, f' Rp {total_salary:,} ', 1, 1, 'R', True)
    
    pdf.ln(15)
    pdf.set_font('Arial', 'I', 9)
    pdf.set_text_color(100, 100, 100)
    pdf.cell(0, 5, 'Catatan: Gaji telah ditransfer langsung ke rekening terdaftar karyawan.', 0, 1)
    pdf.ln(20)
    
    pdf.set_font('Arial', '', 10)
    pdf.set_text_color(43, 43, 43)
    pdf.cell(120, 5, '', 0, 0)
    pdf.cell(70, 5, 'Disetujui Oleh,', 0, 1, 'C')
    pdf.ln(20)
    
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(120, 5, '', 0, 0)
    pdf.cell(70, 5, 'Manajemen Kopi Sibei', 0, 1, 'C')
...
@app.route('/api/payroll/<int:id_payroll>/send-email', methods=['POST'])
@role_required('manager')
def send_payroll_email_route(id_payroll):
    conn = None
    try:
        conn = get_db()
        with conn.cursor() as cur:
            cur.execute(
                "SELECT p.id_penggajian, p.periode, p.rate_per_shift, p.total_shift, p.total_gaji, p.bukti_tf, p.tanggal_dibuat, "
                "u.nama AS cashier_name, u.email AS cashier_email "
                "FROM penggajian p JOIN users u ON p.id_kasir = u.id_user "
                "WHERE p.id_penggajian = %s",
                (id_payroll,)
            )
            payroll = cur.fetchone()
            
        if not payroll:
            return jsonify({"status": "error", "message": "Data penggajian tidak ditemukan."}), 404
            
        recipient = payroll['cashier_email']
        if not recipient or "@" not in recipient:
            return jsonify({"status": "error", "message": "Email kasir tidak valid atau tidak terdaftar."}), 400
            
        cashier_name = payroll['cashier_name']
        ...
        issued_date = payroll['tanggal_dibuat'].strftime("%d %B %Y") if isinstance(payroll['tanggal_dibuat'], (datetime, date)) else str(payroll['tanggal_dibuat'])
        rate = payroll['rate_per_shift']
        total_shifts = payroll['total_shift']
        total_salary = payroll['total_gaji']
        bukti_tf = payroll['bukti_tf']
        
        pdf_bytes = generate_payroll_pdf(cashier_name, recipient, period_label, issued_date, rate, total_shifts, total_salary)
        pdf_base64 = base64.b64encode(pdf_bytes).decode('utf-8')
        
        attachments = [
            {
                "filename": f"slip_gaji_{cashier_name.replace(' ', '_')}_{period}.pdf",
                "content": pdf_base64,
        ...
        bukti_tf_section = ""
        if bukti_tf:
            mime, data, ext = parse_data_url(bukti_tf)
            if data:
                attachments.append({
                    "filename": f"bukti_transfer_{period}.{ext}",
                    "content": data,
                    "contentType": mime,
                    "contentId": "bukti_transfer"
                })
                if "image" in mime:
                    bukti_tf_section = f"""
                    <div style="margin-top: 25px; border-top: 1px dashed #e1dcd6; padding-top: 20px;">
                        <p style="font-size: 14px; font-weight: bold; margin-bottom: 10px; color: #4e3629;">Bukti Transfer:</p>
                        <p style="font-size: 12px; color: #8e7c77; margin-bottom: 15px;">Bukti transfer pembayaran gaji Anda juga dilampirkan pada email ini.</p>
                        <img src="cid:bukti_transfer" alt="Bukti Transfer" style="max-width: 100%; border-radius: 6px; border: 1px solid #e1dcd6;">
                    </div>
                    """
        ...
        <head>
            <meta charset="utf-8">
            <title>Slip Gaji Kopi Sibei</title>
        </head>
        <body style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background-color: #f6f5f3; margin: 0; padding: 20px; color: #2b2b2b;">
            <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; background-color: #ffffff; border: 1px solid #e1dcd6; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
                <tr>
                    <td style="background-color: #4e3629; padding: 30px 40px; text-align: center; color: #ffffff;">
                        <h1 style="margin: 0; font-size: 24px; font-weight: bold; letter-spacing: 1px;">KOPI SIBEI</h1>
                        <p style="margin: 5px 0 0; font-size: 14px; opacity: 0.8;">Slip Gaji & Konfirmasi Pembayaran</p>
                    </td>
                </tr>
                <tr>
                    <td style="padding: 40px;">
                        <h2 style="margin-top: 0; font-size: 18px; color: #4e3629;">Halo {cashier_name},</h2>
                        <p style="font-size: 14px; line-height: 1.6; margin-bottom: 20px;">
                            Terima kasih atas kerja keras dan kontribusi Anda di Kopi Sibei. Bersama email ini, kami lampirkan **Slip Gaji** Anda untuk periode **{period_label}** dalam format PDF.
                        </p>
                        
                        <table cellpadding="0" cellspacing="0" width="100%" style="background-color: #fcf9f6; border: 1px solid #e1dcd6; border-radius: 6px; margin-bottom: 30px; font-size: 14px;">
                            <tr>
                                <td style="padding: 15px 20px; border-bottom: 1px solid #e1dcd6;"><strong>Karyawan:</strong></td>
                                <td style="padding: 15px 20px; border-bottom: 1px solid #e1dcd6; text-align: right;">{cashier_name}</td>
                            </tr>
                            <tr>
                                <td style="padding: 15px 20px; border-bottom: 1px solid #e1dcd6;"><strong>Periode:</strong></td>
                                <td style="padding: 15px 20px; border-bottom: 1px solid #e1dcd6; text-align: right;">{period_label}</td>
                            </tr>
                            <tr>
                                <td style="padding: 15px 20px; border-bottom: 1px solid #e1dcd6;"><strong>Total Shift Hadir:</strong></td>
                                <td style="padding: 15px 20px; border-bottom: 1px solid #e1dcd6; text-align: right;">{total_shifts} Shift</td>
                            </tr>
                            <tr style="font-weight: bold; color: #4e3629; font-size: 16px;">
                                <td style="padding: 15px 20px;"><strong>Total Gaji ditransfer:</strong></td>
                                <td style="padding: 15px 20px; text-align: right;">Rp {total_salary:,}</td>
                            </tr>
                        </table>
                        ...
                        <p style="font-size: 14px; line-height: 1.6; margin-top: 25px;">
                            Slip gaji resmi dan bukti transfer telah dilampirkan pada email ini. Jika ada ketidaksesuaian atau pertanyaan terkait rincian gaji, silakan hubungi manajemen Kopi Sibei.
                        </p>
                        <p style="font-size: 14px; line-height: 1.6; margin-top: 20px;">
                            Salam hangat,<br>
                            <strong>Manajemen Kopi Sibei</strong>
                        </p>
                    </td>
                </tr>
                <tr>
                    <td style="background-color: #fcf9f6; padding: 20px; text-align: center; border-top: 1px solid #e1dcd6; font-size: 12px; color: #8e7c77;">
                        <p style="margin: 0;">Email ini dikirim secara otomatis oleh sistem manajemen internal Kopi Sibei.</p>
                        <p style="margin: 5px 0 0;">&copy; 2026 Kopi Sibei. All Rights Reserved.</p>
                    </td>
                </tr>
            </table>
        </body>
        ...
        subject = f"[Kopi Sibei] Slip Gaji - {cashier_name} - {period_label}"
        success = send_email_via_resend(recipient, subject, html_content, attachments=attachments)
        
        if success:
            return jsonify({"status": "success", "message": f"Slip gaji berhasil dikirim ke email {recipient}!"}), 200
        else:
